...
if (d.activityLevel === "sedentary") {
  penalty += 20; // era 25
  reasons.push({ code: "ACTIVITY_SEDENTARY", message: "Sedentarismo reduz eficiência do déficit e aumenta dificuldade de adesão." });
} else if (d.activityLevel === "light") {
  penalty += 15;
  reasons.push({ code: "ACTIVITY_LIGHT", message: "Baixa atividade limita ritmo de progresso e aumenta dependência de dieta perfeita." });
...
if (d.trainingFrequency < 3) {
  penalty += 5;
  reasons.push({ code: "TRAINING_FREQUENCY_LOW", message: "Treino <3x/sem reduz estímulo mínimo para progresso consistente." });
}

// Sem musculação penaliza
if (!d.strengthTraining) {
  penalty += 5;
  reasons.push({ code: "STRENGTH_TRAINING_MISSING", message: "Sem musculação: maior risco de perder massa magra e piorar composição corporal." });
}

// Educação quando a execução não sustenta a declaração
if (d.activityLevel === "moderate" && (d.trainingFrequency < 3 || !d.strengthTraining)) {
  reasons.push({ code: "ACTIVITY_MODERATE_INCONSISTENT", message: "Atividade moderada exige consistência (frequência e musculação) para ser real na prática." });
}
